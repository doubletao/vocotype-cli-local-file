...
from app.funasr_config import MODEL_REVISION, MODELS, SENSEVOICE_MODEL, SENSEVOICE_LANGUAGES
...
class FunASRServer:
    def __init__(self, engine="sensevoice"):
        self.asr_model = None
        ...
        self._engine = engine  # "paraformer" 或 "sensevoice"
    ...
    def _load_sensevoice_model(self):
        """加载 SenseVoice 多语言模型（中英日粤韩）"""
        try:
            from funasr import AutoModel

            logger.info("开始加载 SenseVoice 模型: %s", SENSEVOICE_MODEL)
            try:
                model_dir = get_model_cache_path(
                    SENSEVOICE_MODEL,
                    revision=None,
            ...
            logger.info("使用 funasr.AutoModel 加载 SenseVoice（PyTorch 后端）")
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info("正在并行初始化FunASR模型（引擎=%s）...", self._engine)
    ...
    def transcribe_audio(self, audio_path, options=None):
        ...
        try:
            ...
            if self._engine == "sensevoice":
                return self._transcribe_sensevoice(audio_path, default_options)
            else:
                return self._transcribe_paraformer(audio_path, default_options)
    ...
    def _transcribe_sensevoice(self, audio_path, options):
        """使用 SenseVoice 进行多语言转录"""
        import re

        language = options.get("language", "auto")
        if language not in SENSEVOICE_LANGUAGES:
            logger.warning("SenseVoice 不支持语言 %s，回退到 auto", language)
            language = "auto"

        use_itn = options.get("use_punc", True)

        # 用 librosa 加载音频为 numpy 数组（避免依赖 ffmpeg）
        try:
            import librosa
        ...
        start = time.time()
        asr_result = self.sensevoice_model.generate(
            input=audio_np,
            language=language,
            use_itn=use_itn,
        )
        ...
        # funasr.AutoModel.generate() 返回 [{'key': ..., 'text': '<|xx|>...实际文本'}]
        raw_text = ""
        if isinstance(asr_result, list) and len(asr_result) > 0:
            raw_text = str(asr_result[0].get("text", ""))
        else:
        ...
        logger.info("SenseVoice 识别完成（语言=%s）: %s", language, final_text[:100])
    ...
    def _transcribe_paraformer(self, audio_path, options):
        """使用 Paraformer 进行中文转录（原有逻辑）"""
        # VAD 处理
        if options["use_vad"] and self.vad_model:
            vad_result = self.vad_model(audio_path)
            logger.info("VAD处理完成，检测到 %s 个语音段", len(vad_result[0]) if vad_result else 0)
        elif options["use_vad"] and not self.vad_model:
        ...
        if isinstance(asr_result, list) and len(asr_result) > 0:
            ...
            if isinstance(first_item, dict) and "text" in first_item:
                ...
                raw_text = str(preds[0]) if isinstance(preds, tuple) and len(preds) > 0 else str(preds)
            else:
        ...
        # 标点恢复
        final_text = raw_text
        if options["use_punc"] and self.punc_model and raw_text.strip():
            try:
